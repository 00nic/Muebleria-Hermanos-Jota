...
router.get("/", (req, res) => { res.status(200).json(catalogo); })
//get devuelve por id 
router.get("/:id", (req, res, next) => { try { const producto = catalogo.find(p => p.id == parseInt(req.params.id)); if (!producto) { return res.status(404).json({ mensaje: "Producto no encontrado" }) } res.status(200).json(producto); } catch (err) { next(err); } })
//post 
